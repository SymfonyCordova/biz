...
class SimpleValidator
{
    ...
    {
        return (bool) preg_match('/^(\d{16}|\d{18}|\d{19})$/', $value);
    }
    ...
    public static function chinese($value){
        return (bool) preg_match('/([\x81-\xfe][\x40-\xfe])/', $value);
    }

    public static function explain($value, array $option = array())
    {
        $option = array_merge(
            array('minLength' => 30, 'maxLength' => 500),
            $option
        );

        $pattern = sprintf("/.{%s,%s}/", $option['minLength'], $option['maxLength']);
